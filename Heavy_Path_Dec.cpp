...
void dfs1(int p, int dep = 1)
{
    ...
    int ma = 0; // 寻找重子节点
    depth[p] = dep; // 当前节点深度
    for (int i = head[p]; i; i = edge[i].next) { // 遍历链式前向星
...
void build()
{
    N = 1 << __lg(n) + 1;
    for (ll i = 1; i <= n; i++)
        tree[N + dfsn[i]] = val[i]; // 以dfs序建立线段树
    for (ll i = N - 1; i; i--)
...
void update(ll l, ll r, ll k)
{
    ...
    for (l += N - 1, r += N + 1; l ^ r ^ 1; l >>= 1, r >>= 1, len <<= 1) {
        tree[l] += cntl * k, tree[r] += cntr * k;
        if (~l & 1)
            tree[l ^ 1] += k * len, mark[l ^ 1] += k, cntl += len;
        if (r & 1)
            tree[r ^ 1] += k * len, mark[r ^ 1] += k, cntr += len;
    }
...
int query_path(int x, int y)
{
    ...
    while (top[x] != top[y]) { // 不在一个路径
        if (depth[top[x]] > depth[top[y]]) { // 深链上跳
            ans += query(dfsn[top[x]], dfsn[x]); // 查询链上区间和（深链节点连续）
            x = fa[top[x]];
        } else {
...
{
    return query(dfsn[x], subtree_max_dfsn[x]);
}
